  ...
  const rows = allRows.map(row => {
    ...
    return {
      json: {
        ...
        key_events: parseInt(row.metricValues[0].value),
  ...
  const table = {
    ...
    schema: {
      fields: [
        ...
        {name: 'key_events', type: 'INTEGER', mode: 'NULLABLE'},
...
function insertRowsToBigQuery_daterange_(projectId, datasetId, tableId, rows) {
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    const values = batch.map(row => {
      const r = row.json;
      return `('${r.date}', '${escapeString_(r.google_ads_campaign_name)}', '${escapeString_(r.google_ads_ad_group_name)}', '${escapeString_(r.google_ads_query)}', '${escapeString_(r.event_name)}', ${r.key_events}, '${r.fetched_at}')`;
    }).join(',\n');
